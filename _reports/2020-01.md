...
The [Yocto Project](https://www.yoctoproject.org/) reported that they have reproducible cross-built binaries working that are independent of both the underlying host distribution the build is run on and independent of the path used for the build. This is now being continually tested on the Yocto Project's automated infrastructure to ensure this state is maintained in the future.
